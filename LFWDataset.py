...
class LFWDataset(Dataset):
    ...
    def flip_image(sample):
        ...
        t_method = random.choice([Image.NONE, Image.FLIP_LEFT_RIGHT])

        if t_method == Image.FLIP_LEFT_RIGHT:
            # transpose image
            f_img = f_img.transpose(t_method)
    ...
    def change_brightness(sample, bright_noise=0.5):
        ...
        s_img = sample['img']
        s_lmrk = sample['lmarks']
        s_img = ImageEnhance.Brightness(s_img).enhance(random.uniform((1-bright_noise), 1))
    ...
    #     sample_image_path = os.path.join(self.image_dir, self.lfw_list[idx]['image_path'])
    #     sample_image = np.asarray(Image.open(sample_image_path), dtype=np.float32) / 255.
    #     sample_landmarks = np.asarray(self.lfw_list[idx]['landmarks'], dtype=np.float32)
    #     self.show_image(sample_image, sample_landmarks)
    #
    # def show_list(self, indices):
    #     str = ''
    ...
    #         str += '=>idx={}:\n image_path: {}\n bbox: {}\n landmarks: {}\n'.format(
    #             idx,
    #             self.lfw_list[idx]['image_path'],
    #             self.lfw_list[idx]['bbox'],
    #             self.lfw_list[idx]['landmarks']
    #         )
    #     print(str)
...
if __name__ == "__main__":
    ...
    lfw_test_anno_path = os.path.join(lfw_lab_dir, "LFW_annotation_test.txt")
